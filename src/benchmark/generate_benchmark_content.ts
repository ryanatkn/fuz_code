...
export const generate_large_content = (
	...
	for (let i = 0; i < multiplier; i++) {
		...
		if (lang === 'ts' || lang === 'js') {
			...
			// Vary component names, props, and variables
			section = section
				.replace(/export let prop/g, `export let prop${i}`)
				.replace(/let variable/g, `let variable${i}`)
				.replace(/class="/g, `class="class${i}_`)
				.replace(/id="/g, `id="id${i}_`)
				.replace(/bind:value=/g, `bind:value${i}=`)
				.replace(/on:click=/g, `on:click${i}=`);
